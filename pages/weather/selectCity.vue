<template>
	<view class="page">
		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{ height: winHeight + 'px' }">
			...
			<view class="tui-list search-result" v-if="inputShowed">
				<view
					...
					:key="index"
					@tap="selectCitys(item)"
					:data-name="item.citys"
					:hover-stay-time="150"
				>
					<view class="tui-list-cell-navigate">{{ item.citys }}</view>
				</view>
			</view>
			<view v-if="!inputVal">
		...
		<view class="tui-indexed-list-alert" v-if="touchmove && lists[touchmoveIndex].letter">{{ lists[touchmoveIndex].letter }}</view>
	</view>
</template>
	...
	methods: {
		...
		searchCity() {
			...
			if(this.inputVal!=''){
				...
				this.$http.post(this.$appurl.getAreaByName, data).then(res => {
					if (res.result.length > 1) {
						for (let index in res.result) {
							var citys = res.result[index].areaName + '·' + res.result[index].cityName + '·' + res.result[index].provinceName;
							if(res.result[index].cityName == null){
								citys = res.result[index].areaName
							}else if (res.result[index].provinceName == null) {
								citys = res.result[index].areaName + '·' + res.result[index].cityName;
							}
							result.push({citys:citys,areaCode: res.result[index].areaCode});
						}
					} else {
						console.log(res.result[0])
						var cityes = res.result[0].areaName + '·' + res.result[0].cityName + '·' + res.result[0].provinceName;
						if(res.result[0].cityName == null){
							cityes = res.result[0].areaName
						}else if (res.result[0].provinceName == null) {
							cityes = res.result[0].areaName + '·' + res.result[0].cityName;
						}
						result.push({citys:citys,areaCode: res.result[0].areaCode});
					}
