<template>
	<view>
		...
		<button :class="['buttonBorder',!_rotate?'dlbutton':'dlbutton_loading']" :style="{'background':bgColor, 'color': fontColor}"
		 style="border: 0px;">
			<view :class="_rotate?'rotate_loop':''" style="line-height: 3.2;">
				<text v-if="_rotate" class="cuIcon cuIcon-loading1 "></text>
				<text v-if="!_rotate" style="font-size: 16px;">{{ text }}</text>
			</view>
		</button>
	</view>
</template>
	...
	export default {
		props: {
			...
			bgColor: {
				//按钮背景颜色
				type: String,
				default: "linear-gradient(to right, #FBD855, #FBD855)",
			},
			fontColor: {
				//按钮字体颜色
				type: String,
...
<style>
	...
	.buttonBorder {
		border: none;
		border-radius: 2.5rem;
		-webkit-box-shadow: 1px 6px 12px 0 rgba(251, 216, 85, 0.6196078431372549);
		box-shadow: 1px 6px 12px 0 rgba(251, 216, 85, 0.6196078431372549);
